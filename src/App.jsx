...
function WatchRedirect() {
  ...
  return React.createElement('div', { style: { padding: '100px', textAlign: 'center', color: '#fff' } }, 'Redirecting...')
}
...
function DetailsPage({ watchlist, onWatchlistChange, isInWatchlist }) {
  ...
  useEffect(() => {
    ...
    const epSlugForUrl = currentEp?.name ? slugify(currentEp.name) : ''
    let canonicalUrl = ''
    if (isEpisodeView) {
      canonicalUrl = 'https://zero-tv.pages.dev/' + type + '/' + id + '/' + slug + '/season/' + seasonNum + '/episode/' + episodeNum + (epSlugForUrl ? '/' + epSlugForUrl : '')
    } else {
      canonicalUrl = 'https://zero-tv.pages.dev/' + type + '/' + id + (slug ? '-' + slug : '')
    }
    const epName = currentEp?.name || ''
    const fullTitle = isEpisodeView
     ? (title + ' S' + seasonNum + 'E' + episodeNum + ' ' + epName + ' - Watch Online - Zero TV').trim()
  ...
  }, [details, type, id, seasonNum, episodeNum, isPlaying, seasonDetails])
    ...
    async function fetchSeason() {
      ...
      try {
        ...
        if (data.episodes && data.episodes.length > 0) {
          if(!urlEpisode) setEpisodeNum(data.episodes[0].episode_number)
        }
      } catch (e) {
      ...
      {!isMovie && (
        <div style={{ padding: '0 28px 20px' }}>
          ...
          <div style={{ marginTop: '10px' }}>
            ...
            {episodesLoading? <div style={{ color: '#888' }}>Loading episodes...</div> : seasonDetails && seasonDetails.episodes? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '10px' }}>
                {seasonDetails.episodes.map(ep => {
                  ...
                  <button key={ep.id} onClick={() => {
                    ...
                    }} style={{ display: 'flex', gap: '12px', padding: '10px', borderRadius: '10px', border: episodeNum === ep.episode_number? '2px solid #e50914' : '1px solid #2a323f', background: episodeNum === ep.episode_number? '#1e242e' : '#151a23', color: '#fff', cursor: 'pointer', textAlign: 'left', alignItems: 'center' }}>
...
function SearchPage({ watchlist, onWatchlistChange, isInWatchlist }) {
  ...
  function handleSelect(item) {
    sessionStorage.setItem('zero-tv-last-path', location.pathname + location.search)
    const slug = slugify(item.title || item.name)
    const type = item.type === 'movie'? 'movie' : item.isAnime? 'anime' : 'tv'
    navigate('/' + type + '/' + item.id + '/' + slug, { state: { from: location.pathname } })
  }
  return (
    <div>
...
function WatchlistPage({ watchlist, onWatchlistChange, isInWatchlist }) {
  ...
  function handleSelect(item) {
    sessionStorage.setItem('zero-tv-last-path', location.pathname)
    const slug = slugify(item.title || item.name)
...
function MoviesWrapper({ watchlist, onWatchlistChange, isInWatchlist }) {
  ...
  function handleSelect(item) {
    ...
    const slug = slugify(item.title || item.name)
    navigate('/movie/' + item.id + '/' + slug, { state: { from: location.pathname } })
  }
...
function TVWrapper({ watchlist, onWatchlistChange, isInWatchlist }) {
  ...
  function handleSelect(item) {
    ...
    sessionStorage.setItem('zero-tv-scroll', window.scrollY.toString())
    const slug = slugify(item.title || item.name)
    navigate('/tv/' + item.id + '/' + slug, { state: { from: location.pathname } })
  }
...
function AnimeWrapper({ watchlist, onWatchlistChange, isInWatchlist }) {
  ...
  function handleSelect(item) {
    ...
    sessionStorage.setItem('zero-tv-scroll', window.scrollY.toString())
    const slug = slugify(item.title || item.name)
    navigate('/anime/' + item.id + '/' + slug, { state: { from: location.pathname } })
  }
