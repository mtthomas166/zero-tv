...
export const api = {
  ...
  nowPlaying: function() { return tmdbFetch('/movie/now_playing'); },
  upcoming: function() { return tmdbFetch('/movie/upcoming'); },
  ...
  onTheAir: function() { return tmdbFetch('/tv/on_the_air'); },
  airingToday: function() { return tmdbFetch('/tv/airing_today'); },
  ...
  airingNowAnime: function() { return tmdbFetch('/discover/tv?with_genres=16&sort_by=popularity.desc'); },
  upcomingAnime: function() { return tmdbFetch('/discover/tv?with_genres=16&sort_by=popularity.desc'); },
...
export function buildUniqueContent(movie) {
  ...
  var director = movie.credits && movie.credits.crew ? (movie.credits.crew.find(function(c){return c.job==='Director';})||{}).name || '' : '';
  return {
    longDesc: title + ' ' + (year ? '(' + year + ')' : '') + ' is a ' + rating + '/10 title. ' + (movie.overview || '') + ' Starring ' + cast + '. ' + (director ? 'Directed by ' + director + '.' : '') + ' Watch ' + title + ' in HD on Zero TV.',
    whyWatch: 'Why watch ' + title + '? Rated ' + rating + ', starring ' + cast + '. ' + (movie.overview || '').slice(0,150),
    faqs: [
      { q: 'Where to watch ' + title + '?', a: 'You can watch ' + title + ' free on Zero TV in HD.' },
      { q: 'What is the rating of ' + title + '?', a: title + ' has ' + rating + '/10 with ' + (movie.vote_count || 0) + ' votes.' },
